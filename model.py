...
class Model():
  def __init__(self, args, infer=False):
    ...
    if tf.__version__.startswith('1.2'):
        ...
        stacked_rnn =[tf.contrib.rnn.DropoutWrapper(cell(args.rnn_size), output_keep_prob = args.keep_prob) if (infer == False and args.keep_prob < 1) else cell(args.rnn_size) \
                      for _ in range(args.num_layers)]
        cell_bw = tf.nn.rnn_cell.MultiRNNCell(cells=stacked_rnn)
        ...
        stacked_rnn =[tf.contrib.rnn.DropoutWrapper(cell(args.rnn_size), output_keep_prob = args.keep_prob) if (infer == False and args.keep_prob < 1) else cell(args.rnn_size) \
                      for _ in range(args.num_layers)]
        cell_bw = tf.contrib.rnn.MultiRNNCell(cells=stacked_rnn)
    ...
    self.state_in =tf.identity((zero_state, zero_state), name='state_in')

    self.num_mixture = args.num_mixture
    NOUT = 1 + self.num_mixture * 6 # end_of_stroke + prob + 2*(mu + sig) + corr

    with tf.variable_scope('rnnlm'):
      output_w = tf.get_variable("output_w", [2* args.rnn_size, NOUT])
    ...
    outputs, state_out =tf.nn.bidirectional_dynamic_rnn(
        ...
        dtype =tf.float32)
    # outputs, state_out =tf.contrib.rnn.static_rnn(cell, inputs, dtype =tf.float32, scope='rnnlm')
    ...
    #flat_target_data = [tf.squeeze(flat_target_data_, [1]) for flat_target_data_ in flat_target_dat
    ...
    def get_lossfunc(z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr, z_eos, x1_data, x2_data, eos_data):
      ...
      # the same effect as:if eos_data ==1, z_eos; else if eos_data ==0, 1-z_eos
  ...
  def sample(self, sess, num=1200):
    ...
    # prev_state_fw = sess.run(self.cell_fw.zero_state(1, tf.float32))
    # prev_state_bw = sess.run(self.cell_bw.zero_state(1, tf.float32))
    ...
    #   feed = {self.input_data: prev_x, self.state_in_fw:prev_state_fw, self.state_in_bw:prev_state_bw}
    ...
    #   prev_state_fw, prev_state_bw = next_state
